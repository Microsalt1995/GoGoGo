...
func TestSlices(t *testing.T) {
	...
	//要创建一个长度不为 0 的空 slice，需要使用内建函数 make。 这里我们创建了一个长度为 3 的 string 类型的 slice（初始值为零值）。
	s := make([]string, 3)
	fmt.Println("emp:", s)
	...
	//除了基本操作外，slice 支持比数组更丰富的操作。比如 slice 支持内建函数 append， 该函数会返回一个包含了一个或者多个新值的 slice。
	s = append(s, "d")
	s = append(s, "e", "f")
	fmt.Println("apd:", s)
